...
@api_router.get("/games/{game_id}/streamers")
async def get_game_streamers(game_id: str):
    """Получить стримеров для конкретной игры"""
    # Получить первый доступный аккаунт для доступа к API
    account = await db.accounts.find_one({"is_authenticated": True})
    if not account:
        raise HTTPException(status_code=400, detail="Нет аутентифицированных аккаунтов")
    
    streamers = await get_streamers_for_game(game_id, account["access_token"])
    return {
        "game_id": game_id,
        "streamers": streamers,
